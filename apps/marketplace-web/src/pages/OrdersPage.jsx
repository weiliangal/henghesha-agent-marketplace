...
export default function OrdersPage() {
    ...
    try {
      await apiRequest(`/orders/${orderId}`, {
        ...
        body: {
          ...
          paymentRemark: "企业已完成线下转账，并向平台提交人工确认。",
        },
      });
      setMessage("已提交支付回执，等待管理员确认。");
      await loadOrders();
    } catch (error) {
  ...
  const stats = [
    { icon: <Inbox size={18} />, label: "全部订单", value: orders.length },
    { icon: <TimerReset size={18} />, label: "处理中", value: orders.filter((item) => item.status === "pending").length },
    { icon: <CreditCard size={18} />, label: "待支付确认", value: orders.filter((item) => item.paymentStatus === "submitted").length },
    { icon: <CheckCircle2 size={18} />, label: "已完成", value: orders.filter((item) => item.status === "completed").length },
  ];
    ...
    <section className="section-shell">
      <div className="hero-panel p-6 sm:p-8 lg:p-10">
        <div className="relative z-10">
          <SectionHeader
            eyebrow="订单中心"
            title="把状态、支付与交付进度放在同一个高可读界面里"
            description="不同角色看到的动作不同，但页面语言保持一致。企业可提交支付回执，学校可标记交付完成，管理员则在后台完成审核闭环。"
      ...
      <div className="mt-8 space-y-6">
        {filteredOrders.length ? (
            ...
            <OrderCard key={order.id} order={order}>
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div className="text-sm leading-7 text-ink/58">
                  {user.role === "enterprise" ? "企业可以在订单推进后提交线下支付回执。" : null}
                  {user.role === "school" ? "学校在确认交付完成后，可以把订单状态推进到完成。" : null}
                  {user.role === "admin" ? "管理员请前往后台统一审核订单与支付状态。" : null}
                </div>
                <div className="flex flex-wrap gap-3">
                  {user.role === "enterprise" && order.paymentStatus === "manual_pending" ? (
          ...
          <EmptyState
            title="当前没有匹配的订单"
            description="可以调整筛选条件，或者先去模板中心和智能体库发起新的采购需求。"
            action={
