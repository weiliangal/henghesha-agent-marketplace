export default function SectionHeader({ eyebrow, title, description, align = "left", action = null }) {
  ...
  return (
    <div className={centered ? "mx-auto max-w-3xl text-center" : "flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"}>
      ...
      {action ? <div className={centered ? "mt-6 flex justify-center" : "lg:shrink-0"}>{action}</div> : null}
    </div>
  );
}
